...
//Player - Constructor
Player::Player(Pawn *ppawn, RefCard *refcard, Cards *rolecard, vector<PlayerCard*> p_hand){
    playerpawn=ppawn;
...
vector<PlayerCard*>  Player::getHand(){
    return player_hand;
}
...
void Player::setHand(vector<PlayerCard*> &ha){
    player_hand=ha;
    Notify(0);
}
...
void Player::drawpcards(int nbcardsdraw, vector<PlayerCard*> &plyrdeck, vector<PlayerCard*> &discardpile, vector<PlayerCard*> &eventCardsAvail){
    
    Notify(4); //display pawn info to identify the player
    for(int k=0;k<nbcardsdraw;k++){
        if(plyrdeck.back()!=nullptr){
            string s1=plyrdeck.back()->getCardName();
            string s2="Event";
            if(s1=="Epidemic Card" ){
                Notify(3); //Notify EpidemicCard;
                //insert function triggers Epidemic Event
            }
            else if(s1.string::find(s2) != string::npos){
            ...
            cout<<"ERR: player deck last card nullptr"<<endl;
        }
    }
...
void Player::discardCards(){
        ...
        while(handsize>7){
            ...
            while (cardnb < 1 || cardnb > handsize) {
                cout << "Enter a valid number from 1 to "<< handsize << ":";
                cin >> cardnb;
                cin.clear();
            ...
            if(cardnb>=0 && cardnb<=handsize){
                CString cardn= player_hand.at(cardnb-1)->getCardName();
                delete player_hand.at(cardnb-1);
                player_hand.erase(player_hand.begin() + (cardnb-1));
                cout<<cardn<<" was succesfully removed"<<endl;
            }
...
void Player::DirectFlight(City* acities[]){
    CString currentcity=this->getPawn()->getPawnCity()->getCityName();
    for(int i=0;i<player_hand.size();i++){
        if(currentcity==player_hand[i]->getCardName()){
            int citychoice=0;
            cout<<"Direct Flight - What city do you want to fly to?"<<endl;
            for(int k=0; k<48;k++){
                
                
                
                cout<<"["<<k+1<<"] "<<acities[k]->getCityName()<<endl;
            }
            
            while(citychoice>48 || citychoice<1)
            {
                cout<<"Enter the number corresponding to the city you want to fly to: "<<endl;
                cin>>citychoice;
                cin.clear();
                cin.ignore(numeric_limits<streamsize>::max(), '\n');
            }
            ...
            CString CStringcity=acities[citychoice-1]->getCityName();
            this->getPawn()->setPawnCityCString(CStringcity, acities);
...
void Player::CharterFlight(City* acities[]){
    //TODO: if event card
    cout<<"Charter Flight - What city do you want to fly to?"<<endl;
    cout<<"You can choose any city corresponding to one of your city cards in your hand."<<endl;
    
    //get all the city cards from player hand
    string strevent="Event";
    vector<int> vcitycardsphand; //vector of index of city cards (to discard event cards) in p_hand
    for(int i=0;i<player_hand.size();i++){
        string pcardname=player_hand[i]->getCardName();
        if(pcardname.string::find(strevent) != string::npos){
            ;
        }else{
            vcitycardsphand.push_back(i);
           // cout<<"["<<i+1<<"] " <<player_hand[i]->getCardName()<<endl;
        }
       
    }
    
    for(int j=0;j<vcitycardsphand.size();j++){
        cout<<"["<<j+1<<"] " <<player_hand[vcitycardsphand[j]]->getCardName()<<endl;
    }
    ...
    CString choicecity=player_hand[indexphand]->getCardName();
    this->getPawn()->setPawnCityCString(choicecity, acities);
...
void Player::treatDisease(int *remainingDiseaseCubes, bool* isCured, bool* isEradicated){
	...
	CString dColor = currentCity->getColor();
	int colorIndex;
	if (dColor == "blue") {
		colorIndex = 0;
	}
...
void Player::ShareKnowledge(vector<Player*> vectorplayers){
...
//or take the City card that matches the city you are in from another player
    ...
    for(int i=0;i<vectorplayers.size();i++){
        ...
        if(cityotherplayer==cityplayer && this!=vectorplayers[i]){
            ...
            CString cname = cityplayer->getCityName();
            ...
            for(int j=0; j<phs;j++){
                if(player_hand[j]->getCardName()==cname){
                    ...
                    cout<< "You have the City card '"<<cname<<"' that matches the city you are in."<<endl;
                    char response='a';
                    do
                    {
                        cout<< "Do you want to give it to "<< vectorplayers[i]->getPlayerName()<<"? [y/n]"<<endl;
                        cin >> response;
                    }
                    while( !cin.fail() && response!='y' && response!='n' && response!='Y' && response!='N' );
                    ...
                    if(response=='y' || response=='Y'){
                        ...
                        cout<<"City card: "<<cname<<" was successfully given to "<<vectorplayers[i]->getPlayerName()<<endl;
                    }
                    else{
            ...
            for(int k=0; k<phs2;k++){
                vector<PlayerCard*> otherplayershand =vectorplayers[i]->getHand();
                if(otherplayershand[k]->getCardName()==cname){
                    //trigger event matching cards
                    
                    cout<< vectorplayers[i]->getPlayerName()<< "has the City card '"<<cname<<"' that matches the city you both are in."<<endl;
                    char response='a';
                    do
                    {
                        cout<< "Do you want take this card from "<< vectorplayers[i]->getPlayerName()<<"? [y/n]"<<endl;
                        cin >> response;
                    }
                    while( !cin.fail() && response!='y' && response!='n' && response!='Y' && response!='N' );
                    ...
                    if(response=='y' || response=='Y'){
                        ...
                        cout<<"City card: "<<cname<<" was successfully taken from "<<vectorplayers[i]->getPlayerName()<<endl;
                    }
                    else{
...
Dispatcher::Dispatcher(Pawn *ppawn, RefCard *refcard, Cards *rolecard, vector<PlayerCard*> p_hand):
...
Medic::Medic(Pawn *ppawn, RefCard *refcard, Cards *rolecard, vector<PlayerCard*> p_hand):
Player(ppawn, refcard, rolecard, p_hand)
{playername="Medic";}
...
void Medic::removeCuredCubes(int* remainingDiseaseCubes, bool* isCured, bool* isEradicated) {
	...
	CString dColor = this->getPawn()->getPawnCity()->getColor();
	int colorIndex;
	if (dColor == "blue") {
		colorIndex = 0;
	}
...
Scientist::Scientist(Pawn *ppawn, RefCard *refcard, Cards *rolecard, vector<PlayerCard*> p_hand):
Player(ppawn, refcard, rolecard, p_hand)
...
Researcher::Researcher(Pawn *ppawn, RefCard *refcard, Cards *rolecard, vector<PlayerCard*> p_hand):
Player(ppawn, refcard, rolecard, p_hand)
...
Operationsexpert::Operationsexpert(Pawn *ppawn, RefCard *refcard, Cards *rolecard, vector<PlayerCard*> p_hand):
Player(ppawn, refcard, rolecard, p_hand)
...
void Operationsexpert::moveResearchstationCity(City* toCity, vector<PlayerCard*> *discardPile)
{
	for (unsigned i = 0; i < player_hand.size(); i++)
...
Quarantinespecialist::Quarantinespecialist(Pawn *ppawn, RefCard *refcard, Cards *rolecard, vector<PlayerCard*> p_hand):
Player(ppawn, refcard, rolecard, p_hand)
...
Contingencyplanner::Contingencyplanner(Pawn *ppawn, RefCard *refcard, Cards *rolecard, vector<PlayerCard*> p_hand):
Player(ppawn, refcard, rolecard, p_hand)
