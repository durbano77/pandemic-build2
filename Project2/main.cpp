...
void initInfectionDeck() {
    ...
    shuffle(infectiondeck.begin(), infectiondeck.end(), default_random_engine(random_device()()));
}
void setInitPlayerDeck(){
    ...
    shuffle(playerdeck.begin(), playerdeck.end(), default_random_engine(random_device()()));
...
void initialInfection() {
	...
	while (i >= 1) {
		for (int k = 3; k > 0; k--) {
			...
			CString city = curr_inf->getCardName();
			CString color = curr_inf->getCardTextFront();
			//infect the city
			for (int j = 1; j <= i; j++) {
...
int main(){
   ...
   // cin.get(); // get one more char from the user
    ...
    //arrayofPlayers[0]->getPawn()->setPawnCityCString("Pekin", cityarr);
    //CString sa=arrayofPlayers[0]->getPawn()->getPawnCity()->getCityName();
    //cout<<sa<<endl;
